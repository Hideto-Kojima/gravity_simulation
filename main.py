...
def update(frame, sc, lines, positions):
    ...
    return (sc,) + tuple(lines)

if __name__ == "__main__":
    num_particles = 3
    num_steps = 10000000
    dt = 0.1
    masses = np.array([40.0, 50.0, 30.0])
    ...
    ani = FuncAnimation(fig, update, frames=len(particle_positions), fargs=(sc, lines, particle_positions), interval=1, blit=True)
    plt.show()
